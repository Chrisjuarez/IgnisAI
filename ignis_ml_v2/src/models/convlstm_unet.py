...
Constructor signature is backward-compatible with train_nautilus.py:
    ConvLSTMUNet(Cd, Cs, hidden=64, lstm_layers=2, drop=0.1)
...
ARCH_VERSION  = "v2"     # Bumped so trainer can namespace the checkpoint file.
...
class ConvLSTMUNet(nn.Module):
    ...
    def __init__(self, Cd, Cs, hidden=64, lstm_layers=2, drop=0.1):
        ...
        self.dyn_enc1 = conv_block(Cd, h1, drop)           # [B*T, h1, H, W]
        self.dyn_down1 = Down(h1, h2, drop)                # [B*T, h2, H/2, W/2]
        self.dyn_down2 = Down(h2, h3, drop)                # [B*T, h3, H/4, W/4]

        # --- Static branch encoder ---
        self.stat_enc1  = conv_block(Cs, h1, drop)         # [B, h1, H, W]
        self.stat_down1 = Down(h1, h2, drop)               # [B, h2, H/2, W/2]
        self.stat_down2 = Down(h2, h3, drop)               # [B, h3, H/4, W/4]
        ...
        self.aspp = ASPP(h3, h3, rates=(1, 2, 4), drop=drop) if USE_ASPP else nn.Identity()
        ...
        # --- Decoder ---
        self.up2 = Up(in_c=h3, skip_c=h2, out_c=h2, drop=drop)   # -> [B, h2, H/2, W/2]
        self.up1 = Up(in_c=h2, skip_c=h1, out_c=h1, drop=drop)   # -> [B, h1, H,   W  ]
